...
import 'package:jumper/src/Features/company/job_details/bloc/services/publish_request_service.dart';

class PublishRequestRepo {
  final PublishRequestService _publishRequestService =
      Get.put(PublishRequestService());

  Future<DataState<bool>>? publish({required int jobId}) async {
    try {
      final httpResponse = await _publishRequestService.publish(
        jobID: jobId,
      );
