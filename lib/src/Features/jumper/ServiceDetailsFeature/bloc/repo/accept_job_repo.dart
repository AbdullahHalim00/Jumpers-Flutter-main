...
import 'package:jumper/src/Features/jumper/ServiceDetailsFeature/bloc/services/accept_job_api_service.dart';

class AcceptJobRepo {
  final AcceptJobApiService _acceptJobApiService =
      Get.put(AcceptJobApiService());

  Future<DataState<bool>>? acceptJob({required int jobId}) async {
    try {
      final httpResponse = await _acceptJobApiService.accept(
        jobID: jobId,
      );
